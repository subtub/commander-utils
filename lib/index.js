    ...
    if (tmp.update !== undefined && tmp.update !== false) {
      commander
        ...
        .action(function(options) {
          ...
          request(tmp.update.packageJsonUrl, function (error, response, body) {
            if (!error && response.statusCode == 200) {
              var latestVersion = JSON.parse(body).version;
              if (options.check) {
                update.checkIfNewVersion(packageJson.version, latestVersion);
              } else {
                if (update.checkIfNewVersion(packageJson.version, latestVersion)) {
                  // TODO: use native node.js (child_process...)
                  //var uninstall = exec('sudo npm uninstall', ['-g', tmp.name]);
