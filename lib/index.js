...
var CommanderUtils = function(commander, packageJson) {
  ...
  /**
   * Parse the commands and options. This we must call before the `program.parse` function.
   *
   * **Example**
  ...
  commander.parseUtils = function() {
    ...
    if (tmp !== undefined && tmp.description !== undefined) {
      descriptionLog(commander, tmp.description, tmp.bugs.url);
    } else {
      descriptionLog(commander, packageJson.description, tmp.bugs.url);
    }
